...
const OptionWheel = ({
    ...
    /* Merged .option-wheel + .option-wheel--dragging */
    ...
    /* Merged .option-wheel__item (+ --right variant) - the --ow-p variable is
       still written per-frame by the rAF loop, so color-mix keeps working. */
    const itemStyle = (index: number): CSSProperties => ({
        ...
        lineHeight: 1,
        fontWeight: selectedIndex === index ? 500 : 200,
        transformOrigin: side === 'left' ? 'left center' : 'right center',
        cursor: 'pointer',
        willChange: 'transform, opacity, filter',
        // --ow-p goes 0 -> 1 as an option approaches the middle of the wheel
        color: `color-mix(in srgb, ${activeColor} calc(var(--ow-p, 0) * 100%), ${textColor})`
    });
