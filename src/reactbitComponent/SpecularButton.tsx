...
/* Size variants (previously .specular-button--sm/md/lg) */
...
/* Replaces color-mix(in srgb, var(--sb-tint) calc(var(--sb-tint-opacity) * 100%), transparent) */
...
const SpecularButton = ({
    ...
    // :active / :focus-visible / :disabled equivalents
    ...
    useEffect(() => {
        ...
        /* Previously .specular-button__fx canvas { display: block; width: 100%; height: 100%; } */
        ...
        const update = (now: number) => {
            ...
            const target =
                pointerAngle != null && p.followMouse && (!p.autoAnimate || proximityT > 0)
                    ? pointerAngle
                    : idleAngle;
    ...
    /* :focus-visible heuristic - outline only when focusing via keyboard (Tab) */
    ...
    /* Merged .specular-button + :active + :focus-visible + :disabled + size variants */
    const buttonStyle: CSSProperties = {
        ...
        ...(focusVisible
            ? {
                outline: `2px solid ${hexToRgba(textColor, 0.6)}`,
                outlineOffset: 3
            }
            : {})
    };
    ...
    return (
        <button
            ...
            style={buttonStyle}
        >
            {/* Merged .specular-button__fx - canvas extends past the button so the rim glow can bleed outside the edge */}
            <span
                ref={fxRef}
                className="specular-button__fx"
                aria-hidden="true"
                style={{ position: 'absolute', inset: -20, pointerEvents: 'none', zIndex: 1 }}
            />
            {/* Merged .specular-button__label */}
            <span className="specular-button__label" style={{ position: 'relative', zIndex: 2 }}>
                {children}
            </span>
        </button>
    );
